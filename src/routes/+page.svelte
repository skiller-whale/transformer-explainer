<script lang="ts">
	...
	const applyExampleDefaults = () => {
		const defaults = exampleDefaults[$selectedExampleIdx];
		if (defaults) {
			blockIdxTemp.set(defaults.block);
			blockIdx.set(defaults.block);
			attentionHeadIdxTemp.set(defaults.head);
			attentionHeadIdx.set(defaults.head);
		}
	};
	...
	const subscribeInputs = (tokenizer: PreTrainedTokenizer) => {
		const runModelOrCache = ({ newValue }) => {
		...
		const unsubscribeInputText = inputText.subscribe((value) => {
			runModelOrCache({ newValue: value });
		});
